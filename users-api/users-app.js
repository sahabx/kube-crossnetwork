...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: `Creating the user failed - please try again later. email is: ${email} environment variable: ${process.env.AUTH_ADDRESS}` });
  }
});
...
app.post('/login', async (req, res) => {
  ...
  // const response = { status: 200, data: { token: 'dulmy token' } };
